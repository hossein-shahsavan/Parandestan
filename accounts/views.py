        ...
        if phone and password:
            ...
            if user.exists():
                return Response({'status': False,
                                 'detail': 'Phone Number already have accounts associated. Kindly try forgot password'},
                                status=HTTP_400_BAD_REQUEST)
            else:
                old = PhoneOTP.objects.filter(phone__iexact=phone)
                if old.exists():
    ...
    def post(self, request, *args, **kwargs):
        ...
        if phone_number:
            ...
            if user.exists():
                ...
                return Response({
                    'status': False,
                    'detail': 'Phone number not recognised. Kindly try a new accounts for this number'
                }, status=HTTP_400_BAD_REQUEST)
